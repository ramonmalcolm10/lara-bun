...
if (pageRoutes.length > 0) {
  ...
  for (const r of sorted) {
    const manifest = manifestByPattern.get(r.urlPattern);
    if (manifest?.domain) {
      domainEntries.push(`  '${r.urlPattern}': '${manifest.domain}',`);
    }
  }

  const domainMapBlock = domainEntries.length > 0
    ? `\nconst domains: Partial<Record<RoutePath, string>> = {\n${domainEntries.join("\n")}\n};\n`
    : `\nconst domains: Partial<Record<RoutePath, string>> = {};\n`;
 ...
 * (including scheme and host) for routes on a different domain.
 *
 * @example
 * route('/')                                    // "/"
 * route('/docs/[slug]', { slug: 'metadata' })   // "/docs/metadata"
 * route('/onboarding')                          // "http://merchant.app.test/onboarding" (from main domain)
 */
